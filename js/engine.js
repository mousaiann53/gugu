const App = {
    data: { profile: { name: '咕咕菌', title: '初级时间管理者', level: 1, rank: '黑铁', skill: '时间暂停 (未解锁)', birthday: '未设置', avatar: null }, gems: 0, stars: 0, eyes: 0, sigils: 0, exp: 0, level: 1, bpExp: 0, tasksDone: {}, bpTasksDone: {}, bpClaimed: [], taskStats: {}, inventory: {}, shopLimits: {}, gachaHistory: [], monthlySavings: 0, lastLogin: '', lastMonth: -1, birthdayClaimedYear: null, lifetimeStats: { totalLogin: 0, totalTasks: 0, totalGacha: 0, totalSavings: 0 }, achievementsClaimed: [] },
    ...
    load() {
        ...
        const def = { profile: { name: '咕咕菌', title: '初级时间管理者', level: 1, rank: '黑铁', skill: '时间暂停 (未解锁)', birthday: '未设置', avatar: null }, gems: 0, stars: 0, eyes: 0, sigils: 0, exp: 0, level: 1, bpExp: 0, tasksDone: {}, bpTasksDone: {}, bpClaimed: [], taskStats: {}, inventory: {}, shopLimits: {}, gachaHistory: [], monthlySavings: 0, lastLogin: '', lastMonth: -1, birthdayClaimedYear: null, lifetimeStats: { totalLogin: 0, totalTasks: 0, totalGacha: 0, totalSavings: 0 }, achievementsClaimed: [] };
        if (s) {
    ...
    hardReset() { if (confirm("⚠ RESET ALL?")) { localStorage.removeItem('gugugu_gold_v7_9'); location.reload(); } },
    ...
    checkBirthday(now, currentYear) { if (this.data.profile && this.data.profile.birthday !== '未设置') { const [bMonth, bDay] = this.data.profile.birthday.split('-'); if (now.getMonth() + 1 == bMonth && now.getDate() == bDay) { if (this.data.birthdayClaimedYear !== currentYear) { this.data.sigils += 10; this.data.birthdayClaimedYear = currentYear; alert("🎂 生日快乐！获得【正印 x10】"); this.save(); } } } },
...
openView(viewId) {
  ...
  if (viewId === 'view-routine') {
    this.renderTabs(
      viewId,
      [{ cn: '每日', en: 'DAILY' }, { cn: '每周', en: 'WEEKLY' }],
      idx => this.renderRoutineList(idx)
    );
  ...
  this.renderHUD();
    ...
    gacha(n, poolType) { let neededSigils = 0; if (this.data.sigils < n) neededSigils = n - this.data.sigils; if (neededSigils > 0) { const gemCost = neededSigils * 180; if (this.data.gems < gemCost) return alert("原石不足！"); if (confirm(`正印不足，消耗 ${gemCost} 原石兑换？`)) { this.data.gems -= gemCost; this.data.sigils += neededSigils; } else return; } this.data.sigils -= n; if (!this.data.lifetimeStats) this.data.lifetimeStats = { totalLogin: 0, totalTasks: 0, totalGacha: 0, totalSavings: 0 }; this.data.lifetimeStats.totalGacha += n; const box = document.getElementById('wish-res'); box.innerHTML = ''; const pool = GUGU_DB.gachaPool[poolType]; for (let i = 0; i < n; i++) { this.data.pity5 = (this.data.pity5 || 0) + 1; let item = null; let roll = Math.random() * 100; let cumulative = 0; for (let pItem of pool) { cumulative += pItem.weight; if (roll <= cumulative) { item = pItem; break; } } if (!item) item = pool[pool.length - 1]; if (item.star >= 5) this.data.pity5 = 0; if (item.type === 'gold') { this.data.monthlySavings += item.val; } else if (item.type === 'item') { if (!this.data.inventory[item.id]) this.data.inventory[item.id] = 0; this.data.inventory[item.id]++; } if (!this.data.gachaHistory) this.data.gachaHistory = []; this.data.gachaHistory.unshift({ name: item.name, star: item.star, date: new Date().toLocaleTimeString() }); if (this.data.gachaHistory.length > 500) this.data.gachaHistory.pop(); const div = document.createElement('div'); div.className = 'pull-line'; div.style.color = item.star >= 5 ? 'var(--gold-main)' : (item.star === 4 ? '#fff' : '#777'); div.innerText = `[${item.star}★] ${item.name}`; box.prepend(div); } this.save(); this.renderWish(poolType === 'limited' ? 0 : 1); },
    renderWish(poolIdx) { const container = document.getElementById('wish-content'); const poolType = poolIdx === 0 ? 'limited' : 'standard'; container.innerHTML = `<div style="padding:20px;"><button class="history-btn" onclick="App.renderHistory()">历史记录</button></div><div style="text-align:center; padding:20px;"><div style="font-family:'Cinzel'; color:#666; margin-bottom:30px;"><span style="color:var(--gold-main); font-size:16px;">180 原石 / 次 (优先消耗正印)</span><br><br>距离五星保底: <span style="color:#fff;">${70 - (this.data.pity5 || 0)}</span></div><div style="display:flex; justify-content:center; gap:20px;"><button class="btn-act" style="padding:5px 30px;" onclick="App.gacha(1, '${poolType}')"><span class="txt-cn">祈愿 x1</span><span class="txt-en">ONCE</span></button><button class="btn-act" style="padding:5px 30px;" onclick="App.gacha(10, '${poolType}')"><span class="txt-cn">祈愿 x10</span><span class="txt-en">TENFOLD</span></button></div><div class="wish-res" id="wish-res"></div></div>`; },
    renderHistory() { const container = document.getElementById('wish-content'); container.innerHTML = `<div style="padding:20px;"><button class="btn-act" onclick="App.renderWish(0)" style="margin-bottom:20px;">BACK</button><div style="max-height:400px; overflow-y:auto;">${(this.data.gachaHistory || []).map(h => `<div style="border-bottom:1px dashed #333; padding:10px; color:${h.star >= 5 ? 'var(--gold-main)' : (h.star === 4 ? '#fff' : '#777')}">[${h.star}★] ${h.name} <span style="float:right; font-size:10px; color:#555">${h.date}</span></div>`).join('')}</div></div>`; },
    renderShop() { const container = document.getElementById('shop-content'); container.innerHTML = `<div class="shop-grid"></div>`; const grid = container.firstChild; GUGU_DB.shop.forEach(s => { const item = GUGU_DB.items[s.itemId]; const bought = this.data.shopLimits[s.id] || 0; let limitText = s.limitType === 'none' ? '不限购' : `剩余: ${s.limit - bought}/${s.limit}`; const canBuy = (s.limitType === 'none' || bought < s.limit) && this.data.stars >= s.price; const div = document.createElement('div'); div.className = 'shop-card'; div.innerHTML = `<div class="item-icon ${item.icon}"></div><span class="txt-cn" style="font-weight:bold">${item.name}</span><span class="txt-en" style="color:#666; font-size:10px;">${item.desc}</span><div style="margin-top:10px; font-size:12px; color:#888;">${limitText}</div><button class="btn-act" ${canBuy ? '' : 'disabled'} onclick="App.buyItem('${s.id}')"><span class="txt-cn">${s.price} 星币</span><span class="txt-en">BUY</span></button>`; grid.appendChild(div); }); },
    buyItem(shopId) { const s = GUGU_DB.shop.find(x => x.id === shopId); const bought = this.data.shopLimits[shopId] || 0; if (s.limitType !== 'none' && bought >= s.limit) return alert("限购已满"); if (this.data.stars < s.price) return alert("星币不足"); this.data.stars -= s.price; if (s.limitType !== 'none') this.data.shopLimits[shopId] = bought + 1; if (s.itemId === 'i02') { this.data.eyes++; alert("树之眼+1"); } else if (s.itemId === 'i05') { this.data.sigils++; alert("正印+1"); } else if (GUGU_DB.vouchers[s.itemId]) { this.data.monthlySavings += GUGU_DB.vouchers[s.itemId]; alert("丰饶资金增加"); } else { if (!this.data.inventory[s.itemId]) this.data.inventory[s.itemId] = 0; this.data.inventory[s.itemId]++; alert("已存入仓库"); } this.save(); this.renderShop(); },
    renderInventory() { const container = document.getElementById('inventory-content'); const invKeys = Object.keys(this.data.inventory); if (invKeys.length === 0) { container.innerHTML = '<div style="padding:50px; text-align:center; color:#555">EMPTY</div>'; return; } container.innerHTML = `<div class="list-wrap"></div>`; const wrap = container.firstChild; invKeys.forEach(id => { const count = this.data.inventory[id]; if (count > 0) { let item = GUGU_DB.items[id] || GUGU_DB.gachaPool.limited.find(g => g.id === id) || GUGU_DB.gachaPool.standard.find(g => g.id === id); if (item) { const div = document.createElement('div'); div.className = 'row'; div.innerHTML = `<div class="row-left"><span class="row-title">${item.name} <span style="color:var(--gold-main)">x${count}</span></span><span class="row-meta">${item.desc || '稀有物品'}</span></div><button class="btn-act" onclick="App.useItem('${id}')"><span class="txt-cn">使用</span><span class="txt-en">USE</span></button>`; wrap.appendChild(div); } } }); },
    useItem(id) { if (confirm("使用此道具吗？")) { this.data.inventory[id]--; if (id === 'i02') { this.data.eyes++; alert("树之眼 +1"); } else if (id === 'i05') { this.data.sigils++; alert("正印 +1"); } else if (GUGU_DB.vouchers[id]) { this.data.monthlySavings += GUGU_DB.vouchers[id]; alert(`丰饶资金 +${GUGU_DB.vouchers[id]}`); } else { alert("道具已使用"); } this.save(); this.renderInventory(); } },
    renderBpPath() { const lvl = Math.floor(this.data.bpExp / 1000); const prog = (this.data.bpExp % 1000) / 10; const container = document.getElementById('bp-content'); const getR = (l) => { if (l === 1) return { t: 'star', v: 1000, n: '星币' }; if (l === 5 || l === 15) return { t: 'eye', v: 5, n: '树之眼' }; if (l === 10) return { t: 'sigil', v: 1, n: '正印' }; if (l === 50) return { t: 'sigil', v: 2, n: '正印x2' }; return { t: 'star', v: 400, n: '星币' }; }; let html = `<div class="bp-bar"><div style="display:flex; flex-direction:column; align-items:center;"><span style="font-family:'Cinzel'; font-size:30px; color:var(--gold-main); line-height:1;">LV.${lvl}</span><span style="font-size:9px; color:#666; font-family:'Cinzel';">EXP ${this.data.bpExp % 1000}/1000</span></div><div class="bp-prog-line"><div class="bp-prog-fill" style="width:${prog}%"></div></div><button class="claim-all-btn" onclick="App.claimAllBp()"><span class="txt-cn" style="color:var(--gold-main)">一键领取</span><span class="txt-en">CLAIM ALL</span></button></div><div class="list-wrap" id="bp-list">`; for (let i = 1; i <= 50; i++) { const r = getR(i); const reached = lvl >= i; const claimed = this.data.bpClaimed.includes(i); html += `<div class="bp-row ${reached ? 'reached' : ''}"><div class="lvl-idx">${i}</div><div style="flex:1; display:flex; align-items:center; gap:10px;"><span style="color:${reached ? '#ddd' : '#555'}; font-size:14px;">${r.n} x${r.v}</span></div><button class="btn-act" ${(!reached || claimed) ? 'disabled' : ''} onclick="App.claimBp(${i}, '${r.t}', ${r.v})"><span class="txt-cn">${claimed ? '已领取' : (reached ? '领取' : '锁定')}</span><span class="txt-en">${claimed ? 'OWNED' : (reached ? 'CLAIM' : 'LOCKED')}</span></button></div>`; } container.innerHTML = html; setTimeout(() => { const l = document.getElementById('bp-list'); const r = l.querySelectorAll('.reached'); if (r.length) r[r.length - 1].scrollIntoView({ block: "center" }); }, 50); },
    claimBp(lvl, type, val) { if (this.data.bpClaimed.includes(lvl)) return; this.data.bpClaimed.push(lvl); if (type === 'star') this.data.stars += val; else if (type === 'eye') this.data.eyes += val; else if (type === 'sigil') this.data.sigils += val; else if (type === 'item') alert("获得实物奖励"); this.save(); this.renderBpPath(); },
    claimAllBp() { const lvl = Math.floor(this.data.bpExp / 1000); let count = 0; for (let i = 1; i <= lvl; i++) { if (!this.data.bpClaimed.includes(i)) { this.data.bpClaimed.push(i); const r = (l) => { if (l === 1) return { t: 'star', v: 1000 }; if (l === 5 || l === 15) return { t: 'eye', v: 5 }; if (l === 10) return { t: 'sigil', v: 1 }; if (l === 50) return { t: 'sigil', v: 2 }; return { t: 'star', v: 400 }; }; const rw = r(i); if (rw.t === 'star') this.data.stars += rw.v; if (rw.t === 'eye') this.data.eyes += rw.v; if (rw.t === 'sigil') this.data.sigils += rw.v; count++; } } if (count > 0) { alert(`已一键领取 ${count} 个奖励`); this.save(); this.renderBpPath(); } },
    renderBpTasks() { const container = document.getElementById('bp-content'); container.innerHTML = `<div style="display:flex; justify-content:center; gap:10px; margin-top:20px;"><button class="btn-act" onclick="App.renderBpSub('daily')"><span class="txt-cn">日记</span><span class="txt-en">DAILY</span></button><button class="btn-act" onclick="App.renderBpSub('weekly')"><span class="txt-cn">周报</span><span class="txt-en">WEEKLY</span></button><button class="btn-act" onclick="App.renderBpSub('monthly')"><span class="txt-cn">月度</span><span class="txt-en">MONTHLY</span></button></div><div class="list-wrap" id="bp-sub"></div>`; this.renderBpSub('daily'); },
    renderBpSub(type) { const list = GUGU_DB.bp.tasks[type]; const box = document.getElementById('bp-sub'); box.innerHTML = ''; list.forEach(t => { const done = this.data.bpTasksDone[t.id]; let actionArea = ''; if (t.req > 0) { const cur = this.data.taskStats[t.type] || 0; const target = t.req; const pct = Math.min((cur / target) * 100, 100); if (done) { actionArea = `<span style="font-size:12px; color:#555">已完成</span>`; } else { actionArea = `<div class="prog-container"><span class="prog-text">(${Math.min(cur, target)}/${target})</span><div class="prog-track"><div class="prog-fill" style="width:${pct}%"></div></div></div>`; } } else { actionArea = `<button class="btn-act" ${done ? 'disabled' : ''} onclick="App.doBpTask('${t.id}', ${t.exp}, '${type}')"><span class="txt-cn">${done ? '已完成' : '完成'}</span><span class="txt-en">${done ? 'DONE' : 'FINISH'}</span></button>`; } const div = document.createElement('div'); div.className = 'row'; div.innerHTML = `<div class="row-left"><span class="row-title">${t.name}</span><span class="row-meta">纪行经验 +${t.exp}</span></div>${actionArea}`; box.appendChild(div); }); },
    doBpTask(id, exp, type) { this.data.bpTasksDone[id] = true; this.data.bpExp += exp; this.save(); this.renderBpSub(type); },
    renderProfile() { const d = this.data.profile; const container = document.getElementById('profile-content'); container.innerHTML = `<div class="profile-card"><div class="avatar-box" onclick="document.getElementById('avatar-input').click()">${d.avatar ? `<img src="${d.avatar}" class="avatar-img">` : `<div class="avatar-placeholder">?</div>`}</div><div class="p-col"><div class="p-row"><span class="p-label">昵称 / NAME</span><span class="p-val">${d.name}<span class="edit-icon" onclick="App.editName()">✎</span></span></div><div class="p-row"><span class="p-label">称号 / TITLE</span><span class="p-val">${d.title}</span></div><div class="p-row"><span class="p-label">等级 / LEVEL</span><span class="p-val">Lv.${this.data.level}</span></div></div><div class="p-col"><div class="p-row"><span class="p-label">生日 / BIRTHDAY</span><span class="p-val">${d.birthday}<span class="edit-icon" onclick="App.editBirthday()">✎</span></span></div><div class="p-row"><span class="p-label">段位 / RANK</span><span class="p-val">${d.rank}</span></div><div class="p-row"><span class="p-label">能力 / ABILITY</span><span class="p-val">${d.skill}</span></div></div></div>`; },
    editName() { const n = prompt("请输入新名字:", this.data.profile.name); if (n) { this.data.profile.name = n; this.save(); this.renderProfile(); } },
    editBirthday() { const n = prompt("设置生日 (格式: MM-DD):", this.data.profile.birthday); if (n) { this.data.profile.birthday = n; this.save(); this.renderProfile(); alert("设置成功"); } },
  ...
  if (a.paused) {
    a.play()
      ...
      .catch(err => {
        ...
        alert('浏览器阻止了音频播放（手机上很常见）。请先随便点一下页面空白处，再点一次 BGM。若仍无声，请打开 F12 控制台查看报错。');
      });
  } else {
    a.pause();
